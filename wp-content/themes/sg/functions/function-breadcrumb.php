	...
	echo '<li>Sie befinden sich hier:</li>';  
		if(is_page() && !is_front_page()){
			...
			$post_anchestors = get_post_ancestors($post);
				...
				echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="'.get_permalink($post->ID).'"><span itemprop="name">'.get_the_title().'</span></a></li>';
		} 
		elseif(is_single()){
			...
			echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="'.get_permalink($post->ID).'" title="'.get_the_title().'"><span itemprop="name">'.get_the_title().'</span></a><meta itemprop="position" content="3" /></li>';
		}
	echo '</ol>';
} ?>
...
if ( isset( $_GET["id"] ) ) {
	...
	$offer = mysql_query("SELECT * FROM wp_gutschein_feed_offers WHERE id = $id LIMIT 1");
	$offer = mysql_fetch_array($offer);

	$shopid = $offer['shopid'];

	$shop = mysql_query("SELECT * FROM wp_gutschein_feed_shops WHERE id = $shopid LIMIT 1");
...
<head>
	...
	<link rel="stylesheet" href="http://sg.sparfuchs-gutschein.de/fuchs.css" type="text/css" media="screen, projection"/>
...
<?php 
	...
	?>
<img alt="<?php echo $shopname ?> Gutscheincode" src="http://i.sparfuchs-gutschein.de/<?php echo strtolower($shopnamelogo); ?>-logo.png" title="<?php echo $shopname ?>"/>
...
<input onclick="select()" class="redirect-code" value="<?php echo !empty($code) ? '' : '';  ?><?php echo $code; ?>" readonly />

<!--<div class="redirect-code">
	<div class="code-iframe">
		 <iframe src="http://www.gutscheinpony.de/go/an-<?php echo $id; ?>.html?position=0?noframe=0"></iframe> 
	</div>
</div>-->
 ...
 <table style="width:100%;position:relative;" cellpadding="2">
  ...
  <tr>
    ...
    <td><?php if (!empty($minimum_order_value)): echo '<span class="mb">' . $minimum_order_value . ' €</span>'; 
  ...
  <tr>
    <td>Gültig für:</td>
    <td><?php if ($new_customers == 'YES' && $existing_customers == 'YES'): echo '<span class="bk">Neu- &amp; Bestandskunden</span>'; 
	  ...
	  <tr>
    <?php if (!empty($description)): echo '<td><strong>Info:</strong></td>'; endif; ?>
    <?php if (!empty($description)): echo '<td><ol>' .$description . '</ol></td>'; endif; ?>
  </tr>
